...
$$ATT = \E[\Delta Y_i | D_i=1] - \E[X_i'|D_i=1]\beta_0$$
...
* Step 2: Combine this estimate with estimates of $\E[\Delta Y_{i,t} | D_i=1]$ and $\E[X_i'|D_i=1]$ (using treated group) to estimate $ATT$

## Covariate Balancing

Alternatively, if we could choose [balancing weights]{.alert} $\nu_0(X_i)$ such that the distribution of $X_i$ was the same in the untreated group as it is in the treated group after applying the balancing weights, then we would have that (from the second term above)
\begin{align*}
...
- This also provides a connection to estimating $ATT$ under conditional parallel trends using machine learning for $p(X_i)$ and $\E[\Delta Y_i|X_i,D_i=0]$ (see: Chang (2020) and Callaway, Drukker, Liu, and Sant'Anna (2023))
